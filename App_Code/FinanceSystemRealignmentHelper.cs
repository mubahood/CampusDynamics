...
public static class FinanceSystemRealignmentHelper
{
    ...
    public static bool EnsureFinanceAdminAccess(Page page, Label messageLabel)
    {
        ...
        {
            RedirectOrJsRedirect(page, "~/Default.aspx");
            return false;
        }
    ...
    /// <summary>
    /// Issues a redirect that is safe to call from any context — including DevExpress
    /// and ASP.NET AJAX callbacks where Response.Redirect throws
    /// "Response.Redirect cannot be called in a Page callback."
    /// During a normal postback or GET, a standard server-side redirect is used.
    /// During any callback (Page.IsCallback == true), a minimal script-based redirect
    /// is written to the response so the browser navigates away cleanly.
    /// </summary>
    private static void RedirectOrJsRedirect(Page page, string url)
    {
        if (page == null)
            return;

        string absoluteUrl = VirtualPathUtility.ToAbsolute(url);

        if (page.IsCallback)
        {
            // Response.Redirect is forbidden inside any callback.
            // Emit a tiny HTML fragment with a script redirect; the browser
            // executes it even inside an AJAX/DevExpress callback response.
            HttpResponse resp = HttpContext.Current != null
                ? HttpContext.Current.Response
                : page.Response;

            resp.Clear();
            resp.ContentType = "text/html";
            resp.Write("<script>window.top.location='" + absoluteUrl + "';</script>");
            resp.End();
        }
        else
        {
            page.Response.Redirect(absoluteUrl, true);
        }
    }
