  ...
  <span class="c-key">"timestamp"</span>: <span class="c-str">"2026-05-16T10:30:00.0000000Z"</span>
            ...
            <div class="api-section" id="authentication">
                ...
                <div class="api-section__body">
                    ...
                    <div class="api-note api-note--warn"><strong>Integration superuser token — <code>xaxu</code>.</strong> A permanent shared secret token, <code>xaxu</code>, is provisioned for server-to-server integrations. It never expires and is a <strong>superuser</strong>: it satisfies <em>any</em> required role or user type, so it is authorised for <strong>every</strong> endpoint and action — including ID card <strong>write</strong> actions (<code>approve</code>, <code>halt</code>, <code>printed</code>, <code>ready</code>, <code>collected</code>, <code>cancel</code>, batch, windows) that otherwise require an <code>idcard_operator</code>/<code>admin</code> role. Use it exactly like any token: <code>?token=xaxu</code> on GET, or in the POST body. Because it bypasses all role checks, treat it as a confidential secret — never expose it in client-side/browser code or public repos. Example: <code>POST /API/v2/idcard.aspx?token=xaxu&amp;action=halt</code> with body <code>{"request_no":"IDR-2026-000003","reason":"There is no picture","action":"halt"}</code>.</div>
