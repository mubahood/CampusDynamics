...
public partial class staff_photo : System.Web.UI.Page
{
    ...
    private void ProcessFileUpload()
    {
        ...
        string staffNo = Request.Form["regno"] ?? Request.QueryString["regno"];
        if (string.IsNullOrEmpty(staffNo))
        {
            SendResponse(false, "Error! Registration No is required");
            return;
        }
        ...
        {
            SendResponse(false, "Error! File size cannot exceed 5MB");
            return;
        }
        ...
        {
            SendResponse(false, "Error! Only image files are allowed");
            return;
        }

        string uploadFolder = "~/COOPERP/StudentInfo/photos/";
        ...
        string safeFileName = curImage + ".jpg";
        ...
        STUD.UpdatePhoto(curImage + ".jpg", staffNo);

        var response = new ApiResponse
        {
            success = true,
            message = "Student Photo File uploaded Successfully",
            fileName = safeFileName,
            filePath = "/COOPERP/StudentInfo/photos/" + safeFileName,
