...
-- Fill a MISSING credit unit from the catalogue; never overwrite a stored one.
-- The stored figure is what the course was worth when it was taken, and a
-- catalogue that holds one value per course cannot express a curriculum that
-- changed. Printing a transcript is a read; it has no business restating the
-- record it is reporting.
UPDATE acad_results r JOIN acad_course c ON c.CourseID = r.courseid
SET r.creditUnits  = c.CreditUnit
WHERE regno = reg AND (r.CreditUnits IS NULL OR r.CreditUnits = 0);
...
BEGIN
  ...
  ELSE
    ...
    SELECT
      ...
      result_comment,
      COALESCE(NULLIF(r.CreditUnits,0), c.CreditUnit, 3),
