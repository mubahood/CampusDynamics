<%@ Page Language="C#" MasterPageFile="~/COOPERP/NewScreens/SidebarMaster.master" AutoEventWireup="true" CodeFile="PublishedMarksController.aspx.cs" Inherits="COOPERP_NewScreens_PublishedMarksController" Title="Published Marks Controller - Admin" %>
...
<div id="modalBatchWizard" class="pm-modal pm-modal--wide">
	...
	<div class="pm-modal__body">
		...
		<div id="wizPanel1" class="pm-wiz-panel">
			<div class="pm-fg"><label>Workflow Action</label><select id="wizAction" class="pm-select"><option value="published">Reprocess published results</option></select></div>
		</div>
...
<div class="pm-card">
	<div class="pm-card__head">
		<div>
			<div class="pm-card__title">Published Marks Controller</div>
			<div class="pm-muted">Published marks for all students</div>
		</div>
	</div>

	<div class="pm-top-controls">
		<button type="button" class="pm-btn pm-btn--primary" onclick="openBatchWizard('published')">Wizard: Batch Reprocess</button>
	</div>
...
<div style="display:none;">
	...
	<div class="pm-filters">
		...
		<div class="pm-fg"><label>Status</label><asp:DropDownList ID="ddlStatus" runat="server" CssClass="pm-select" Enabled="false"><asp:ListItem Value="published" Selected="True">Published Only</asp:ListItem></asp:DropDownList></div>
	...
	<div class="pm-bulk-bar" id="bulkBar"><span class="pm-bulk-label" id="bulkCountLabel">0 selected</span><button type="button" class="pm-btn pm-btn--danger" onclick="openBulkModal('unpublish')">Unpublish Selected</button><button type="button" class="pm-btn pm-btn--primary" onclick="openBulkModal('reprocess')">Reprocess Selected</button><button type="button" class="pm-btn pm-btn--ghost" onclick="clearSelection()">Clear Selection</button></div>
...
function goWithQuery(params){ var query = new URLSearchParams(); Object.keys(params).forEach(function(k){ if(params[k]!==undefined && params[k]!==null) query.set(k, params[k]); }); window.location.href='PublishedMarksController.aspx?'+query.toString(); }
window.applyFilters=function(){ var year=qs('<%= ddlYear.ClientID %>').value, sem=qs('<%= ddlSemester.ClientID %>').value, prog=qs('<%= ddlProg.ClientID %>').value, lect=qs('<%= ddlLecturer.ClientID %>').value, ps=qs('<%= ddlPageSize.ClientID %>').value, q=qs('<%= txtSearch.ClientID %>').value; goWithQuery({pg:'1',year:year,sem:sem,status:'published',prog:prog,lect:lect,ps:ps,q:q}); };
...
window.openBulkModal=function(action){ if(_selectedIds.length===0){showToast('No records selected.','err');return;} _bulkAction=action; var labels={unpublish:'Unpublish '+_selectedIds.length+' records and move them back to pending.',reprocess:'Reprocess '+_selectedIds.length+' published records (regenerate GPA/CGPA)'}; qs('bulkModalTitle').textContent='Bulk '+action.charAt(0).toUpperCase()+action.slice(1); qs('bulkDesc').textContent=labels[action]+'.'; qs('bulkComment').value=''; clearAlert('bulkAlert'); qs('bulkReqNote').style.display='none'; qs('bulkCommentWrap').style.display=action==='unpublish'?'':'none'; if(action==='unpublish'){ qs('bulkComment').placeholder='Optional reason for unpublish...'; } openModal('modalBulk'); };
window.submitBulk=function(){ if(!_bulkAction||_selectedIds.length===0)return; var comment=qs('bulkComment').value.trim(); clearAlert('bulkAlert'); qs('btnBulkConfirm').disabled=true; callAJAX('BulkAction',{ids:_selectedIds,action:_bulkAction,comment:comment},function(d){ qs('btnBulkConfirm').disabled=false; if(d.success){showToast(d.message||'Bulk action completed.','ok');window.closeModal('modalBulk');setTimeout(function(){location.reload();},800);} else showAlert('bulkAlert',d.message||'Bulk action failed.','err'); }); };
...
window.unpublishMark=function(id){ if(!confirm('Unpublish this result and return it to pending stage?')) return; callAJAX('UnpublishMark',{id:id,comment:''},function(d){ if(d.success){showToast(d.message||'Record unpublished.','ok'); setTimeout(function(){location.reload();},800);} else showToast(d.message||'Unpublish failed.','err'); }); };
window.reprocessMark=function(id){ if(!confirm('Reprocess this published result and regenerate GPA/CGPA?')) return; callAJAX('ReprocessPublishedMark',{id:id},function(d){ if(d.success){showToast(d.message||'Result reprocessed.','ok'); setTimeout(function(){location.reload();},800);} else showToast(d.message||'Reprocess failed.','err'); }); };
