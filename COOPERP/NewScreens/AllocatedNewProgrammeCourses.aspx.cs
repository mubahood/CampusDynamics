...
public partial class COOPERP_NewScreens_AllocatedNewProgrammeCourses : System.Web.UI.Page
{
        ...
        string sql = @"
            SELECT
                ...
                COUNT(DISTINCT progcode) AS prog_count
                        ...
                        WHERE UPPER(IFNULL(status,'Active'))='ACTIVE'
                            AND IFNULL(is_lecturere_assigned,'No')='Yes'
                            AND IFNULL(lecturer_id,0) > 0";
    ...
    private void BindGrid()
    {
        ...
        wh.Append(" AND UPPER(IFNULL(pc.status,'Active'))='ACTIVE' AND IFNULL(pc.is_lecturere_assigned,'No')='Yes' AND IFNULL(pc.lecturer_id,0) > 0");
