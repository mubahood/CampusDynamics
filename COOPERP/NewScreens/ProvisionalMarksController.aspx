<%@ Page Language="C#" MasterPageFile="~/COOPERP/NewScreens/SidebarMaster.master" AutoEventWireup="true" CodeFile="ProvisionalMarksController.aspx.cs" Inherits="COOPERP_NewScreens_ProvisionalMarksController" Title="Provisional Marks Controller - Admin" %>
...
.pm-table tr.row--incomplete td:first-child{border-left:3px solid #f97316;}
...
<div id="modalBatchWizard" class="pm-modal pm-modal--wide">
	<div class="pm-modal__head"><span class="pm-modal__title">Batch Publish Wizard</span><button type="button" class="pm-modal__close" onclick="closeModal('modalBatchWizard')">&times;</button></div>
	<div class="pm-modal__body">
		...
		<div id="wizPanel1" class="pm-wiz-panel">
			<div class="pm-fg"><label>Workflow Action</label><select id="wizAction" class="pm-select"><option value="published">Publish to final results</option></select></div>
		</div>
...
<div class="pm-card">
	<div class="pm-card__head">
		<div>
			<div class="pm-card__title">Provisional Marks Publish Queue</div>
			<div class="pm-muted">Pending marks awaiting final publishing</div>
		</div>
	</div>
...
<div class="pm-stats" style="padding:8px 10px;border-bottom:1px solid #edf1f6;">
	...
	<div class="pm-top-controls">
		...
		<button type="button" class="pm-btn pm-btn--success" onclick="filterReady()">&#10003; View Ready to Publish</button>
	</div>
	<div id="readyBanner" class="pm-ready-banner" style="display:none;">
		<strong>&#10003; Filtered: Ready to Publish</strong> &mdash; Showing only records with both CW and Exam marks submitted.
		<button type="button" class="pm-btn pm-btn--ghost" onclick="clearReadyFilter()" style="margin-left:auto;">&#10005; Clear Filter</button>
	</div>
	...
	<div class="pm-filters">
		...
		<div class="pm-fg"><label>Status</label><asp:DropDownList ID="ddlStatus" runat="server" CssClass="pm-select" Enabled="false"><asp:ListItem Value="pending" Selected="True">Pending Publish Only</asp:ListItem></asp:DropDownList></div>
...
function goWithQuery(params){ var query = new URLSearchParams(); Object.keys(params).forEach(function(k){ if(params[k]!==undefined && params[k]!==null) query.set(k, params[k]); }); window.location.href='ProvisionalMarksController.aspx?'+query.toString(); }
window.applyFilters=function(){ var year=qs('<%= ddlYear.ClientID %>').value, sem=qs('<%= ddlSemester.ClientID %>').value, prog=qs('<%= ddlProg.ClientID %>').value, lect=qs('<%= ddlLecturer.ClientID %>').value, ps=qs('<%= ddlPageSize.ClientID %>').value, q=qs('<%= txtSearch.ClientID %>').value; var ready=(new URLSearchParams(window.location.search)).get('ready')==='1'?'1':undefined; goWithQuery({pg:'1',year:year,sem:sem,status:'pending',prog:prog,lect:lect,ps:ps,q:q,ready:ready}); };
window.filterByStatus=function(){ var year=qs('<%= ddlYear.ClientID %>').value, sem=qs('<%= ddlSemester.ClientID %>').value, prog=qs('<%= ddlProg.ClientID %>').value, lect=qs('<%= ddlLecturer.ClientID %>').value, ps=qs('<%= ddlPageSize.ClientID %>').value, q=qs('<%= txtSearch.ClientID %>').value; goWithQuery({pg:'1',year:year,sem:sem,status:'pending',prog:prog,lect:lect,ps:ps,q:q}); };
window.filterReady=function(){ var year=qs('<%= ddlYear.ClientID %>').value, sem=qs('<%= ddlSemester.ClientID %>').value, prog=qs('<%= ddlProg.ClientID %>').value, lect=qs('<%= ddlLecturer.ClientID %>').value, ps=qs('<%= ddlPageSize.ClientID %>').value, q=qs('<%= txtSearch.ClientID %>').value; goWithQuery({pg:'1',year:year,sem:sem,status:'pending',prog:prog,lect:lect,ps:ps,q:q,ready:'1'}); };
window.clearReadyFilter=function(){ var params=new URLSearchParams(window.location.search); params.delete('ready'); params.set('pg','1'); window.location.href='ProvisionalMarksController.aspx?'+params.toString(); };
(function(){ if((new URLSearchParams(window.location.search)).get('ready')==='1'){ var b=qs('readyBanner'); if(b) b.style.display='flex'; } })();
...
window.openReview=function(id){ _id=id; clearAlert('reviewAlert'); qs('reviewDetailGrid').innerHTML='<dt style="font-size:10px;color:#6b7280;">Loading…</dt>'; qs('reviewMarksDisp').innerHTML=''; qs('reviewComment').value=''; qs('reviewPrevComment').style.display='none'; if(qs('btnPublishFromReview')) qs('btnPublishFromReview').disabled=false; openModal('modalReview'); callAJAX('GetProvisionalRecord',{id:id},function(d){ if(!d.success){showToast(d.message||'Error','err');window.closeModal('modalReview');return;} var r=d.record; qs('modalReviewTitle').textContent='Review: '+r.regno+' — '+r.courseID; qs('reviewDetailGrid').innerHTML=dlRow('Reg No',r.regno)+dlRow('Course',r.courseID)+dlRow('Programme',r.prog_id)+dlRow('Academic Year',r.acad_year)+dlRow('Semester','Yr '+(r.study_year||'—')+', Sem '+r.semester)+dlRow('Submitted By',r.submitted_by||'—')+dlRow('Current Status',statusPill(r.provisional_marks_status))+dlRow('Reviewed By',r.provisional_marks_reviewed_by||'—'); qs('reviewMarksDisp').innerHTML=marksBanner(r.provisional_course_work_marks,r.provisional_exam_marks,r.provisional_total_marks); if(r.provisional_marks_review_comments){ qs('reviewPrevComment').style.display='block'; qs('reviewPrevComment').innerHTML='<strong>Previous comment:</strong> '+r.provisional_marks_review_comments; qs('reviewComment').value=r.provisional_marks_review_comments; } var missingR=[]; if(r.provisional_course_work_marks==null) missingR.push('Course Work'); if(r.provisional_exam_marks==null) missingR.push('Exam Marks'); if(missingR.length>0){ if(qs('btnPublishFromReview')) qs('btnPublishFromReview').disabled=true; showAlert('reviewAlert','Cannot publish: '+missingR.join(' and ')+' missing. Both are required.','err'); } }); };
window.submitReview=function(action){ if(!_id)return; var comment=qs('reviewComment').value.trim(); if(action==='rejected'&&!comment){showAlert('reviewAlert','A comment is required when rejecting.','err');return;} clearAlert('reviewAlert'); if(qs('btnPublishFromReview')) qs('btnPublishFromReview').disabled=true; qs('btnReject').disabled=true; callAJAX('ReviewProvisionalMarks',{id:_id,action:action,comment:comment},function(d){ if(qs('btnPublishFromReview')) qs('btnPublishFromReview').disabled=false; qs('btnReject').disabled=false; if(d.success){showToast(action==='published'?'Marks published to final results.':'Marks rejected.','ok');window.closeModal('modalReview');setTimeout(function(){location.reload();},800);} else showAlert('reviewAlert',d.message||'Operation failed.','err'); }); };
window.openPublish=function(id){ _id=id; clearAlert('publishAlert'); qs('publishMarksDisp').innerHTML='<div style="padding:10px;color:#6b7280;font-size:11px;">Loading…</div>'; qs('publishInfo').innerHTML=''; if(qs('btnPublishConfirm')) qs('btnPublishConfirm').disabled=false; openModal('modalPublish'); callAJAX('GetProvisionalRecord',{id:id},function(d){ if(!d.success){showToast(d.message||'Error','err');window.closeModal('modalPublish');return;} var r=d.record; qs('publishMarksDisp').innerHTML=marksBanner(r.provisional_course_work_marks,r.provisional_exam_marks,r.provisional_total_marks); qs('publishInfo').innerHTML='<strong>'+r.regno+'</strong> — '+r.courseID+' &nbsp;|&nbsp; '+r.acad_year+' · Yr '+(r.study_year||'—')+', Sem '+r.semester; var missing=[]; if(r.provisional_course_work_marks==null) missing.push('Course Work'); if(r.provisional_exam_marks==null) missing.push('Exam Marks'); if(missing.length>0){ if(qs('btnPublishConfirm')) qs('btnPublishConfirm').disabled=true; showAlert('publishAlert','Cannot publish: '+missing.join(' and ')+' missing. Both are required.','err'); } }); };
...
window.openBatchWizard=function(action){ syncWizardLookups(); qs('wizAction').value=action||'published'; qs('wizScope').value='selected'; qs('wizScopeFilters').style.display='none'; qs('wizScope').onchange=function(){ qs('wizScopeFilters').style.display=this.value==='programme'?'grid':'none'; }; setWizStep(1); openModal('modalBatchWizard'); };
