...
public partial class COOPERP_NewScreens_TimetableCalendar : Page
{
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static object CalendarData(string acadYear, int campusId, string progcode, int studyYear, int semester, int roomId, int teacherId)
    {
        TimetableService.EnsureSchema();
        ...
        DataTable dt = TimetableService.Query(
            "SELECT it.item_id, it.day_no, TIME_FORMAT(it.start_time,'%H:%i') st, TIME_FORMAT(it.end_time,'%H:%i') et, " +
            "HOUR(it.start_time)*60+MINUTE(it.start_time) sm, HOUR(it.end_time)*60+MINUTE(it.end_time) em, " +
            "TRIM(IFNULL(c.courseName, it.course_code)) course, TRIM(it.course_code) code, IFNULL(it.progcode,'') progcode, IFNULL(it.study_year,0) study_year, IFNULL(it.semester,0) semester, " +
            "IFNULL(it.teacher_id, pc.lecturer_id) eff_teacher, IFNULL(te.emp_name, IFNULL(le.emp_name,'')) teacher, " +
            "IFNULL(it.room_id,0) room_id, IFNULL(r.RoomName,'') room, IFNULL(it.room_label,'') room_label, it.campus_id, IFNULL(cp.campus_short_name,'') campus, it.session_type, it.delivery_mode " +
            "FROM acad_timetable_item it JOIN acad_programmecourses pc ON pc.ID=it.programmecourse_id " +
            "LEFT JOIN acad_course c ON TRIM(c.courseID)=TRIM(it.course_code) LEFT JOIN hrm_employee te ON te.empID=it.teacher_id LEFT JOIN hrm_employee le ON le.empID=pc.lecturer_id " +
            "LEFT JOIN acad_lecturerooms r ON r.RoomID=it.room_id LEFT JOIN acad_campuses cp ON cp.ID=it.campus_id " +
            "WHERE it.status='ACTIVE' AND it.acad_year=@ay" + wh.ToString() + " ORDER BY it.day_no, it.start_time", ps.ToArray());

        List<object> rows = new List<object>();
        foreach (DataRow r in dt.Rows)
            rows.Add(new Dictionary<string, object> {
                { "id", I(r, "item_id") }, { "dayNo", I(r, "day_no") }, { "start", S(r, "st") }, { "end", S(r, "et") },
                { "sm", I(r, "sm") }, { "em", I(r, "em") }, { "course", S(r, "course") }, { "code", S(r, "code") },
                { "progcode", S(r, "progcode") }, { "studyYear", I(r, "study_year") }, { "semester", I(r, "semester") },
                { "teacherId", I(r, "eff_teacher") }, { "teacher", S(r, "teacher") }, { "roomId", I(r, "room_id") },
                { "room", S(r, "room") }, { "roomLabel", S(r, "room_label") }, { "campusId", I(r, "campus_id") }, { "campus", S(r, "campus") },
                { "sessionType", S(r, "session_type") }, { "deliveryMode", S(r, "delivery_mode") } });
