<%@ Page Language="C#" MasterPageFile="~/COOPERP/NewScreens/SidebarMaster.master" AutoEventWireup="true" CodeFile="RequestedNewProgrammeCourses.aspx.cs" Inherits="COOPERP_NewScreens_RequestedNewProgrammeCourses" Title="Requested Programme Courses - Campus Dynamics" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <!-- ── Stat tiles ────────────────────────────────────────── -->
    <div class="pc-stats">
        <div class="pc-stat"><div class="pc-stat__label">Pending Requests</div><div class="pc-stat__value"><asp:Literal ID="litTotal" runat="server" Text="0" /></div></div>
        <div class="pc-stat"><div class="pc-stat__label">Core Courses</div><div class="pc-stat__value"><asp:Literal ID="litCoreCount" runat="server" Text="0" /></div></div>
        <div class="pc-stat"><div class="pc-stat__label">Elective Courses</div><div class="pc-stat__value"><asp:Literal ID="litElectiveCount" runat="server" Text="0" /></div></div>
    ...
    <div class="pc-card">
        <div class="pc-card__head">
            <div class="pc-card__head-main">
                <div class="pc-card__title">Requested Courses (Awaiting Approval) <asp:Literal ID="litRequestAlert" runat="server" /></div>
                <button type="button" class="pc-btn pc-btn--primary pc-btn--sm" onclick="openModal('NEW')">
            ...
            <div class="pc-card__head-filters">
                <div class="pc-filter-bar">
                    <button type="button" class="pc-btn pc-btn--sm" onclick="openBatchModal()">Batch Assign Lecturers</button>
                    <button type="button" class="pc-btn pc-btn--sm" onclick="openImportFromLoadAllocationsWizard()" title="Import lecturer assignments from load allocations">
...
function importStep1_Discover() {
    ...
    fetch('RequestedNewProgrammeCourses.aspx/ImportStep_DiscoverCandidates', {
        method: 'POST',
...
function importStep4_Confirm() {
    ...
    fetch('RequestedNewProgrammeCourses.aspx/ImportStep_ExecuteAssignments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json; charset=utf-8' },
...
function resetFilters(){
    ...
    filterPcTable('');
    window.location.href=window.location.pathname+'?rq=pending';
}
(function initFilterSelects(){
    var qs=new URLSearchParams(window.location.search);
    var yr=qs.get('yr'), sem=qs.get('sem'), asg=qs.get('asg'), lec=qs.get('lec'), tp=qs.get('tp'), rq=qs.get('rq');
    if(!rq){ rq='pending'; }
...
function submitRequestDecision(){
    ...
    fetch('RequestedNewProgrammeCourses.aspx/ProcessAllocationRequestDecision',{
        method:'POST',
        headers:{'Content-Type':'application/json; charset=utf-8'},
...
function loadLecturerFilterOptions(){
    ...
    if(!filterVal) return;
    fetch('RequestedNewProgrammeCourses.aspx/GetLecturerFilterOptions',{
        method:'POST',
        headers:{'Content-Type':'application/json; charset=utf-8'},
        body:'{}'
    })
...
function submitBatchAssignment(){
    ...
    fetch('RequestedNewProgrammeCourses.aspx/BatchAssignLecturers',{
        method:'POST',
        headers:{'Content-Type':'application/json; charset=utf-8'},
