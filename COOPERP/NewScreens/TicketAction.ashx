<%@ WebHandler Language="C#" Class="NewScreens_TicketAction" %>
...
public class NewScreens_TicketAction : IHttpHandler, IRequiresSessionState
{
    ...
    private void HandleDetail(HttpContext ctx)
    {
        ...
        DataRow ticket = SupportTicketDB.GetTicket(id);
        if (ticket == null)
    ...
    private void HandleSetStatus(HttpContext ctx, string adminName)
    {
        ...
        value = value.ToUpper();
        if (!InArray(value, new string[] { "OPEN","IN_PROGRESS","AWAITING_REPLY","RESOLVED","CLOSED" }))
    ...
    private void HandleSetPriority(HttpContext ctx)
    {
        ...
        { ctx.Response.Write("{\"ok\":false,\"msg\":\"Invalid params\"}"); return; }
        value = value.ToUpper();
        if (!InArray(value, new string[] { "LOW","NORMAL","HIGH","URGENT" }))
    ...
    private string BuildBadgesHtml(string status, string issue, string priority)
    {
        ...
        sb.AppendFormat("<span class='tc-badge' style='background:{0};color:{1}'>{2}</span>",
            SupportTicketDB.GetStatusBg(status), SupportTicketDB.GetStatusColor(status),
        ...
        if (priority == "HIGH" || priority == "URGENT")
            sb.AppendFormat("<span class='tc-badge' style='background:rgba(220,53,69,.10);color:{0}'>{1}</span>",
                SupportTicketDB.GetPriorityColor(priority), HttpUtility.HtmlEncode(priority));
        return sb.ToString();
    }

    private string BuildThreadHtml(HttpContext ctx, int ticketId)
    {
        DataTable messages    = SupportTicketDB.GetMessages(ticketId, true);
        DataTable attachments = SupportTicketDB.GetTicketAttachments(ticketId);
        ...
        foreach (DataRow msg in messages.Rows)
        {
            ...
            if (attMap.ContainsKey(msgId))
            {
                foreach (DataRow att in attMap[msgId])
                {
                    ...
                    string url    = portalBase + "/COOPERP/Support/Uploads/" + stored;

                    if (SupportTicketDB.IsImageExtension(ext))
                        sb.AppendFormat(
                            "<div><img src='{0}' alt='{1}' class='tc-att-img' onclick=\"openLightbox('{0}')\"/></div>",
                            HttpUtility.HtmlAttributeEncode(url), HttpUtility.HtmlAttributeEncode(orig));
                    else
                        sb.AppendFormat(
                            ...
                            " {1}</a></div>",
                            HttpUtility.HtmlAttributeEncode(url), HttpUtility.HtmlEncode(orig));
                }
            }
            ...
            sb.Append(HttpUtility.HtmlEncode(sender))
              .Append(" &bull; ").Append(SupportTicketDB.FormatDateTime(ts))
              .Append("</div></div>");
        }
        return sb.ToString();
    }
    ...
    {
        foreach (string s in arr) if (s == val) return true;
        return false;
    }
