...
public partial class COOPERP_NewScreens_DashboardNewProgrammeCourses : System.Web.UI.Page
{
    ...
    private void LoadStats()
    {
        string sql = @"
            SELECT
                SUM(CASE WHEN UPPER(IFNULL(status,'Active'))='ACTIVE' THEN 1 ELSE 0 END) AS total,
                SUM(CASE WHEN UPPER(IFNULL(status,'Active'))='ACTIVE' AND (IFNULL(is_lecturere_assigned,'No')='No' OR IFNULL(lecturer_id,0)=0) THEN 1 ELSE 0 END) AS core_count,
                SUM(CASE WHEN IFNULL(allocation_request_lecturer_id,0) > 0 AND UPPER(IFNULL(allocation_request_admin_status,'Pending'))='PENDING' THEN 1 ELSE 0 END) AS elective_count,
    ...
    private void BindGrid()
    {
        ...
        string qView = (Request.QueryString["view"] ?? "all").Trim().ToLower();
        ...
        if (qView == "active")
        {
            wh.Append(" AND UPPER(IFNULL(pc.status,'Active'))='ACTIVE'");
        }
        else if (qView == "unlocated")
        {
            wh.Append(" AND UPPER(IFNULL(pc.status,'Active'))='ACTIVE' AND (IFNULL(pc.is_lecturere_assigned,'No')='No' OR IFNULL(pc.lecturer_id,0)=0)");
        }
        else if (qView == "requested")
        {
        ...
        // Pager
        string qs = BuildQs(qYear, qSem, qAssigned, qLecturer, qType, qReq, qSearch, qView);
    ...
    private string BuildQs(string yr, string sem, string asg, string lec, string tp, string rq, string q, string view)
    {
        StringBuilder sb = new StringBuilder();
        ...
        if (!string.IsNullOrEmpty(view) && !string.Equals(view, "all", StringComparison.OrdinalIgnoreCase)) sb.Append("&view=" + HttpUtility.UrlEncode(view));
        return sb.ToString();
    }
